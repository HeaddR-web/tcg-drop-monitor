...
**Saturn sperrt Skripte seit 15.09.2026.** Gemessen 19:29 Uhr: `requests` und `curl` bekommen HTTP 403 mit einer Captcha-Seite (7719 Bytes). Der Lauf meldete davon nichts, weil `fetch()` den curl-Body mit Rückgabewert 0 als Seite durchreichte und der Parser darin nichts fand, also stand still "0 relevante Treffer" (vorher 2 bis 4 je Lauf). Drei Änderungen: `fetch()` liest jetzt den echten HTTP-Status von curl (`-w %{http_code}`) und nimmt nur 200 als Seite, alles andere steht als Zeile im Log (erste Fassung war eine Body-Suche nach "captcha", Codex und Grok haben sie am 15.09. beide verworfen: reCAPTCHA im Head einer gültigen Seite hätte sie verworfen, 429 oder 503 wären still durchgerutscht). Saturn läuft wie MediaMarkt AT über `fetch_browser()` (Scrapling Stufe 1 kommt durch: 200, 21 Suchergebnisse). Und die Produktseite für Zielprodukte (PS5 Pro) wird bei Browser-Quellen ebenfalls über den Browser geholt, vorher lief sie immer über `fetch()` und der Status blieb bei gesperrten Shops "unklar" (Codex-Befund). Testlauf mit nur Saturn: 2 relevante Treffer, gleiche Zahl wie vor der Sperre. Um 20 Uhr zog MediaMarkt.de nach (Lauf 19:51: fünfmal 403 mit Captcha, 0 Treffer), seitdem auch dort `browser: True`. Test nur MediaMarkt: 4 Treffer. Saturn stand ohnehin nur in der Mac-Liste, an der Cloud/Mac-Aufteilung ändert sich nichts.

**Amazon-Direktlink in den Einkaufswagen (seit 15.09.2026):** jede Amazon-Meldung trägt neben LINK einen zweiten Link "IN DEN KORB" auf die offizielle Add-to-Cart-Adresse (`warenkorb_link()`, ASIN aus der Produkt-URL, Menge 1). Eingeloggt landet der Artikel mit einem Tipp im Korb, gekauft wird von Hand. Nicht eingeloggt schickt Amazon erst zur Anmeldung und danach in den Korb (gemessen 15.09. über den Browser). Andere Shops haben keine solche Adresse. Testlauf nur Amazon ohne Telegram-Token: 45 von 45 Meldungen mit Korb-Link. Den Tipp auf dem Handy konnte ich nicht selbst prüfen, das ist Stefanies Test.
